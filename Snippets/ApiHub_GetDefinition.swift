...
func sample(
  ...
  let response = try await client.getDefinition(
    request: GetDefinitionRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/apis/\(apiId)/versions/\(versionId)/definitions/\(definitionId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        apiId: "[placeholder]", versionId: "[placeholder]", definitionId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
