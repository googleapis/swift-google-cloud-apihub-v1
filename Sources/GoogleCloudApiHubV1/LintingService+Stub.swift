...
extension Clients {
  ...
  class LintingServiceTransport: LintingServiceStub {
    ...
    public func getStyleGuide(
      ...
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudApiHubV1.StyleGuide.self, from: data)
    }
    ...
    public func updateStyleGuide(
      ...
      let path = try { () throws -> Swift.String in
        guard let pathVariable0 = request.styleGuide.map({ $0.name }), !pathVariable0.isEmpty else {
          throw GoogleCloudGax.RequestError.binding(
            "'request.style_guide.name' is not set or is empty")
        }
        return "/v1/\(pathVariable0)"
      }()
      var query = [
    ...
    public func getStyleGuideContents(
      ...
      let (data, _) = try await self.inner.rpc(for: req).get()
      return try GoogleCloudWkt._ProtoJSONDecoder().decode(
        GoogleCloudApiHubV1.StyleGuideContents.self, from: data)
    }
